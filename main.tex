...
%   UW User ID (mail); the previously completed academic term, the completion
...
\employername{XXXXXXXXXXXXXXXXXXX}
\employerlocation{City, Prov/State, Country}
\authorname{Given Middle Surname}
\studentnumber{XXXXXXXXX}
\userid{XXXXXX}
\term{NX}
\program{NNNNNNNNN}
% uncomment the next line if this is a confidential report
% \confidential{1}
...
Some more text and a cross reference to \Cref{app:firstappx} and remember that one can lie about statistics \pcite{liewithstat}.

Let's cite! The Einstein's journal paper \pcite{einstein} and the Dirac's book \pcite{dirac} are physics related items. 
